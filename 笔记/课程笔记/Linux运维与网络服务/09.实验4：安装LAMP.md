...
```
出现下图表示编译脚本校验完成
![](附件/图片/Pasted image 20251111153131.png)
编译并安装
```
make -j$(nproc)
make install
...
```
![](附件/图片/Pasted image 20251111153359.png)
查看编译安装的版本
![](附件/图片/Pasted image 20251111153503.png)
放行防火墙
```
...
```
网页访问
![](附件/图片/Pasted image 20251111153716.png)
### 部署静态网页
上传静态网页
![](附件/图片/Pasted image 20251111154538.png)
重启服务
访问浏览器
![](附件/图片/Pasted image 20251111154610.png)
### DNS解析
在/var/named/mlishu.local.zone添加www指向服务器的解析
![](附件/图片/Pasted image 20251111154713.png)
编辑完成后重启named服务
如下图就是带域名的解析
![](附件/图片/Pasted image 20251111154937.png)
## 安装MairaDB
### 安装依赖
```
...
```
为了提高编译速度，优先下载fmt包
![](附件/图片/Pasted image 20251118150557.png)
```
...
cmake . \
 ...
 -DWITHOUT_TOKUDB=1
```
![](附件/图片/Pasted image 20251118150202.png)
...
```
![](附件/图片/Pasted image 20251118152931.png)
...
./configure \
 ...
 --enable-fpm
```
![](附件/图片/Pasted image 20251118125459.png)
### 正式编译安装
```
make -j$(nproc)
make install
...
```
![](附件/图片/Pasted image 20251118125959.png)
``
```
